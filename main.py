...
class UserEditorApp(QMainWindow):
    ...
    def on_api_success(self, data, data_type):
        if data_type == 'template':
            ...
            # Este data_type não é mais usado no fluxo principal, mas mantido por segurança
            pass
        ...
        try:
            ...
            for team in teams_list:
                ...
                for assignee in team.get('assignees', []):
                    ...
                    # Usa Nome e Sobrenome como chave, como solicitado
    ...
    def criar_formulario_dinamico(self):
        # Pega a referência para o layout que já existe no scroll_content.
        form_layout = self.scroll_content.layout()
        
        # A verificação "if not form_layout" foi removida, pois ela causava o problema.
        # Agora, simplesmente limpamos o layout existente.
        while form_layout.count():
            child = form_layout.takeAt(0)
            if child.widget():
                child.widget().deleteLater()
        
        # Limpa os dicionários de referência
        self.form_line_edits.clear(); self.form_checkboxes.clear(); self.form_comboboxes.clear()
        
        # Repopula o formulário com os widgets baseados no template
